...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(B, T, self.n_head,
                   self.head_dim).transpose(1, 2)  # (B, nh, T, hs)
        ...
        mask = torch.tril(torch.ones(T, T,
                                     device=x.device)).unsqueeze(0).unsqueeze(
                                         0)  # (1, 1, T, T)
...
class GPT2(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        # Ensure boolean types for linter right before config creation
        config_args['bias'] = bool(config_args.get('bias', True))
        config_args['tie_weights'] = bool(config_args.get('tie_weights', True))
        ...
        # Remap HuggingFace keys to match your model's keys
